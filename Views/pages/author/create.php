...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-md p-8">
        ...
        <form action="/author/articles/create" method="POST" class="space-y-6">
            <div>
                <label for="title" class="block text-sm font-medium text-gray-700 mb-2">
                    Titre de l'article *
                </label>
                <input type="text" id="title" name="title" required
                       value="<?= htmlspecialchars($data['old']['title'] ?? '') ?>"
            ...
            <div>
                ...
                <textarea id="content" name="content" required rows="15"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                          placeholder="Écrivez votre article ici..."><?= htmlspecialchars($data['old']['content'] ?? '') ?></textarea>
            </div>
                        ...
                        <?php foreach ($data['categories'] as $category): ?>
                            <label class="flex items-center space-x-2 cursor-pointer">
                                <input type="checkbox" name="categories[]" value="<?= $category['id'] ?>"
                                       <?= in_array($category['id'], $data['old']['categories'] ?? []) ? 'checked' : '' ?>
                                       class="rounded text-blue-600 focus:ring-blue-500">
                                <span class="text-sm"><?= htmlspecialchars($category['name']) ?></span>
                            </label>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p class="text-gray-500 col-span-full">Aucune catégorie disponible</p>
                    <?php endif; ?>
                </div>
            </div>
